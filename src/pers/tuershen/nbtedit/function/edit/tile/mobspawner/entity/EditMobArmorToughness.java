...
public class EditMobArmorToughness extends EditMobSpawnManager {

    @TagParameter( TYPE = TagType.DOUBLE)
    private double attackDamage;

    public EditMobArmorToughness(){
        this.key = "generic.armorToughness";
        this.slotItem = new ItemStack(Material.MOB_SPAWNER);
        ItemMeta itemMeta = this.slotItem.getItemMeta();
        itemMeta.setDisplayName("§7设置刷怪笼中的实体盔甲韧性");
        List<String> lore = new ArrayList<>();
        itemMeta.setLore(lore);
        this.slotItem.setItemMeta(itemMeta);
    }

    @EditDescribe( describe = {"§eⒺ §2请输入生物实体盔甲韧性，取值范围"+Double.MIN_VALUE+"~"+Double.MAX_VALUE})
    @Override

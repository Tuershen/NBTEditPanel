...
public class EditItemLore extends AbstractEditManager {
    ...
    public EditItemLore(){
        ...
        itemMeta.setDisplayName("§2设置物品的Lore，支持颜色代码，用空格分割表示多行");
        this.slotItem.setItemMeta(itemMeta);
    }


    @EditDescribe( describe = {"§eⒺ §2修改物品Lore，"})
    @Override
    public TagBase inputParams(TagBase tagBase, String... params) {
        ...
        for (int i = 0; i < params.length ; i++) {
            list.add(NewNBTTag.type((byte)8).newTag(params[i]));
        }
        map.put("Lore", list);
